...
import { adminLoginReducer } from '../../auth/utils'
import { Link } from 'react-router-dom'
import admin from '../../images/adminBg/admin.jpg'
...
function Loginform() {
    const [state, dispatch] = useReducer(adminLoginReducer, {email: '', password: ' '})
    const {loginResponse, loginAdmin} = useAdminLogin()
    const navigate = useNavigate()
useEffect(()=>{

    if(loginResponse?.data?.success){
        navigate('/admindashboard')
        localStorage.setItem('adminToken', JSON.stringify(loginResponse.data.token))
...
console.log(loginResponse?.data?.error)
    ...
    const handleSubmit = async (e) =>{
        e.preventDefault();
        const response = await loginAdmin(state)
  ...
  return (
    <>
    <div style={{ backgroundImage: `url("${admin}")` }} className='flex flex-column items-center bg-no-repeat bg-cover bg-center bg-fixed -mt-20 p-20' >
                <form onSubmit={handleSubmit} className='flex flex-column items-center p-8  w-96 rounded-tl-3xl rounded-br-3xl border-solid border-2 border-neutral-50 bg-neutral-50'  >
                    <h1 className='-mt-4 pb-6 text-center '> Admin Login</h1>
                    <div>
